...
export default class RawTxConstructor extends Component {
  ...
  getTxCorrespondingInfo = async (testCase) => {
    ...
    const fromAccount = await oexchain.account.getAccountByName(actionInfo.accountName);
    statusInfo.fromAccount = fromAccount;
    const toAccount = await oexchain.account.getAccountByName(actionInfo.toAccountName);
    ...
    switch (actionType) {
      ...
      case Constant.CREATE_NEW_ACCOUNT:
        const newAccount = await oexchain.account.getAccountByName(actionInfo.payloadDetailInfo.newAccountName);
      ...
      case Constant.ISSUE_ASSET:
        const owner = await oexchain.account.getAccountByName(actionInfo.payloadDetailInfo.owner);
        statusInfo.owner = owner;
        break;
      case Constant.INCREASE_ASSET:
        const increaseAccount = await oexchain.account.getAccountByName(actionInfo.payloadDetailInfo.accountName);
      ...
      case Constant.SET_ASSET_FOUNDER:
        // 先向oexchain.asset转账，然后oexchain.asset减去这笔钱，最后修改资产信息
      ...
      case Constant.VOTE_CANDIDATE:
        const candidate = await oexchain.dpos.getCandidateByName(actionInfo.payloadDetailInfo.accountName);
    ...
    for (const step of procedure) {
      if (step.type == 'get') {
        ...
        try {
          ...
          testResultInfo += '\n交易原始信息：' + originalInfo;
          this.setState({ testResult: testResultInfo });
          const signInfo = await oexchain.oex.signTx(step, step.privateKey);
          const txHash = await oexchain.oex.sendSingleSigTransaction(step, signInfo);
          ...
          while(!gotReceipt) {
            ...
            this.setState({ testResult: testResultInfo });
            this.sleep(3000);
            const receipt = await oexchain.oex.getTransactionReceipt(txHash);
            if (receipt != null) {
              gotReceipt = true;
  ...
  testSendCase = async (testCase) => {
    try {
      ...
      testResultInfo += '\n交易原始信息：' + originalInfo;
      this.setState({ testResult: testResultInfo });
      const signInfo = await oexchain.oex.signTx(testCase, testCase.privateKey);
      const txHash = await oexchain.oex.sendSingleSigTransaction(testCase, signInfo);
      testResultInfo += '\n本次交易发送成功，获得交易hash：' + txHash;
      this.setState({ testResult: testResultInfo });
      let gotReceipt = false;
      while(!gotReceipt) {
  ...
  getResult = (method, params) => {
    const dataToSrv = JSON.stringify({ jsonrpc: '2.0',
      ...
      params,
      id: 1 });
    return oexchain.utils.postToNode({
      data: dataToSrv,
    });
  }
