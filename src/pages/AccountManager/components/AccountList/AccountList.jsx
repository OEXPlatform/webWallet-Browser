...
import { Dialog, Button, Table, Input, Tag, Balloon, Message, NumberPicker, Icon, Grid } from '@alifd/next';
...
const {Row, Col} = Grid;
const addAccountImg = require('./images/ID_icon_addaccount.png');
const importAccountImg = require('./images/ID_icon_Importaccount.png');
...
export default class AccountList extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      authorListFooter: (<view>
                        <Button style={styles.dialogFullBtn} type='primary' onClick={this.submitAllAuthorUpdate.bind(this)}>{T('提交所有修改')}</Button>
                      </view>),
      txListFooter: (<view>
                        <Button style={styles.dialogOKBtn} type='primary' onClick={this.onUpdateTx.bind(this)}>{T('刷新')}</Button>
                        <Button style={styles.dialogCancelBtn} type='normal' onClick={this.onTxClose.bind(this)}>{T('取消')}</Button>
                      </view>),
      txFeeInfo: '',
      helpVisible: false,
  ...
  componentDidMount = async () => {
    try {
      oexchain.dpos.getDposInfo().then(dposInfo => this.state.dposInfo = dposInfo);
      //oexchain.oex.getChainConfig().then(chainConfig => this.state.chainConfig = chainConfig);
      ...
      const assetInfo = await oexchain.account.getAssetInfoById(this.state.chainConfig.sysTokenID);
      ...
      oexchain.dpos.getDposIrreversibleInfo().then(irreversibleInfo => this.state.irreversibleInfo = irreversibleInfo);

      this.state.keystoreList = utils.loadKeystoreFromLS();
      eventProxy.on('updateKeystore', () => {
        this.state.keystoreList = utils.loadKeystoreFromLS();
      });
  ...
  checkBlockRollback = async (blockHash, blockNum) => {
    const blockInfo = oexchain.oex.getBlockByNum(blockNum);
  ...
  onImportAccountOK = async () => {
    ...
    if (this.state.idReg.test(this.state.importAccountName)) {
      ...
      try {
        oexchain.account.getAccountByName(this.state.importAccountName).then(account => {
          if (account != null) {
            const accountInfos = [...self.state.accountInfos, account];
            self.setState({ accountInfos, importAccountVisible: false });
            self.saveAccountsToLS();
  ...
  showAssets = (index) => {
    ...
    for (const balance of balances) {
      if (this.state.assetInfos[balance.assetID] === undefined) {
        promiseArr.push(oexchain.account.getAssetInfoById(balance.assetID));
      }
  ...
  */
  showTxs = async (index) => {
    try {
      ...
      this.state.curBlock = await oexchain.oex.getCurrentBlock(false);
      this.state.curAccount = this.state.accountInfos[index];
      for (const balance of this.state.accountInfos[index].balances) {
        if (this.state.assetInfos[balance.assetID] == null) {
          const assetInfo = await oexchain.account.getAssetInfoById(balance.assetID);
      ...
      if (allTxInfoSet != null) {
        ...
        if (accountTxInfoSet == null) {   // 如果在本地不存在此账号的交易信息，则从链上同步其所有交易
          await oexchain.oex.getCurrentBlock(false).then(async (curBlock) => {
        ...
        for (const txInfo of accountTxInfoSet.txInfos) {
          ...
          for (const actionInfo of txInfo.actions) {
            if (this.state.assetInfos[actionInfo.assetID] === undefined) {
              const asset = await oexchain.account.getAssetInfoById(actionInfo.assetID);
  ...
  updateTxStatus = async (txInfo, lastIrreveribleBlockNum, callback) => {
    if (Object.prototype.hasOwnProperty.call(txInfo, 'txStatus') && txInfo.txStatus != Constant.TxStatus.NotExecute && txInfo.txStatus != Constant.TxStatus.SendError) {   // 通过txStatus判断交易receipt是否已经获取过，条件成立则同步区块状态，否则获取交易的receipt
      ...
      const receipt = await oexchain.oex.getTransactionReceipt(txInfo.txHash);
      ...
      const internalTx = await oexchain.oex.getInternalTxByHash(txInfo.txHash);
      if (internalTx != null) {
  ...
  */
  syncTxFromNode = async () => {
    try {
      ...
      const self = this;
      oexchain.oex.getCurrentBlock(false).then(async (curBlock) => {
        ...
        for (const account of self.state.accountInfos) {
          ...
          for (; blockNum < curBlockNum; blockNum += step) {
            ...
            promiseArr.push(oexchain.oex.getTxsByAccount(accountName, blockNum, lookForward));
            // 内部交易
            accountInternalTxPromiseArr.push(oexchain.oex.getInternalTxsByAccount(accountName, blockNum, lookForward));
          }
          ...
          for (const txHash of accountTxs) {
            if (accountExistTxs[txHash] == null) {
              promiseArr.push(oexchain.oex.getTransactionByHash(txHash));                             
            }
          }
          ...
          for (let txInfo of txInfos) {
            if (blockCache[txInfo.blockHash] == null) {
              promiseArr.push(oexchain.oex.getBlockByHash(txInfo.blockHash));
            }
          }
  ...
  syncTxsOfAccount = async (account, curBlockNum) => {
    ...
    for (; blockNum >= account.number; blockNum -= step) {
      ...
      promiseArr.push(oexchain.oex.getTxsByAccount(account.accountName, blockNum, lookBack + 1));
      // TODO 内部交易
      //accountTxs.push(oexchain.oex.getInternalTxsByAccount(accountName, blockNum, lookBack));
    }
  ...
  exportPrivateKey = (index) => {
    this.state.curAccount = this.state.accountInfos[index];
    const publicKey = this.state.curAccount['authors'][0]['owner'];
    const allKeys = keystore.getAllKeys();
    let i = 0;
    for (; i < allKeys.length; i++) {
      if (allKeys[i].publicKey == publicKey) {
        break;
      }
    }
    if (i == allKeys.length) {
      Feedback.toast.error('因本地不存在此账户公钥所对应的私钥，因此无法导出');
      return;
    }
    Feedback.toast.success('输入密码后可导出私钥');
  }
  ...
  onSystemCreatAccountOK = () => {
    if (!this.state.newAccountReg.test(this.state.oexchainAccount)) {
      Feedback.toast.error(T('账号格式错误'));
      return;
    }
    if (this.state.oexchainPublicKey == '') {
    ...
    // this.props.createAccountBySystem({ accountName: this.state.oexchainAccount, publicKey: this.state.oexchainPublicKey, email: this.state.email });
  };
  ...
  onSelfCreateAccountOK = async () => {
    ...
    const exist = await oexchain.account.isAccountExist(this.state.newAccountName);
    if (exist) {
    ...
    this.state.curAccount = await oexchain.account.getAccountByName(this.state.creator);
    ...
    this.state.txInfo = { actionType: Constant.CREATE_NEW_ACCOUNT,
      accountName: this.state.curAccount.accountName,
      toAccountName: this.state.chainConfig.accountName,  // oexchain.account
        ...
        setTimeout(() => {
          oexchain.oex.getTransactionReceipt(result.msg).then(receipt => {
            ...
            if(receipt != null) {
              ...
              if (status == 1) {
                oexchain.account.getAccountByName(self.state.newAccountName).then(account => {
                  if (account != null) {
  ...
  onBindNewAuthorOK = async () => {
    ...
    if (newOwner.indexOf('.') > 0 || newOwner.length <= 8) {
      const exist = await oexchain.account.isAccountExist(newOwner);
      if (!exist) {
  ...
  onWithdrawTxFeeOK = async () => {
    ...
    if (this.state.txFeeType == TxFeeType.Asset) {
      const assetInfo = await oexchain.account.getAssetInfoByName(txFeeName);
      const assetId = assetInfo.assetId == null ? 0 : assetInfo.assetId;
      payload += oexchain.utils.getContractPayload('withdrawAssetFee', ['uint256'], [assetId]);
    } else if (this.state.txFeeType == TxFeeType.Contract) {
      const account = await oexchain.account.getAccountByName(txFeeName);
      payload += oexchain.utils.getContractPayload('withdrawAccountFee', ['uint256'], [account.accountID]);
    } else if (this.state.txFeeType == TxFeeType.Coinbase) {
      const account = await oexchain.account.getAccountByName(txFeeName);
      payload += oexchain.utils.getContractPayload('withdrawCoinbaseFee', ['uint256'], [account.accountID]);
    }
  ...
  getTxFee = async () => {
    const txFeeName = this.state.txFeeType == TxFeeType.Asset ? this.state.assetName : this.state.curAccount.accountName;
    const txFeeInfoObj = await oexchain.fee.getObjectFeeByName(txFeeName, this.state.txFeeType);
    ...
    if (txFeeInfoObj == null) {
      ...
      if (this.state.txFeeType == TxFeeType.Asset) {
        ...
        const accountInfo = await oexchain.account.getAccountByName(txFeeName);
  ...
  symbolRender = (value) => {
    ...
    const self = this;
    oexchain.account.getAssetInfoById(value).then(assetInfo => {
  ...
  onTransferOK = () => {
    ...
    const self = this;
    oexchain.account.isAccountExist(this.state.transferToAccount).then(exist => {
      if (!exist) {
  ...
  onAddContractABIOK = () => {
    if (!utils.isEmptyObj(this.state.contractABI) && !oexchain.utils.isValidABI(this.state.contractABI)) {
  ...
  render() {
    return (
      <div>
        {/* <div style={{margin: '55px 0 0 72%'}}>
          
        </div> */}
        <IceContainer style={{...styles.table, width: '78%', height: '50%', margin: '0px 11% 20px 11%' }}>
          <Row align='center'>
            <Col>
          ...
          <hr/>
          <Table language={T('zh-cn')} primaryKey="accountName" 
                 isZebra={false}  hasBorder={false} 
                 dataSource={this.state.accountInfos}>
            <Table.Column
              width={80}
              title="ID"
              dataIndex="accountID"
            ...
            />
            <Table.Column title={T("操作")} width={300} cell={this.renderOperation} />
          </Table>
          {/* <div onClick={this.addAccountBySystem.bind(this)} style={styles.addNewItem}>
            + 新增账户(第三方免费帮您创建)
          </div> */}
          {/* <Button type='primary' style={styles.btn} onClick={this.addAccountBySelf.bind(this)}>
            <img style={styles.img} src={addAccountImg}/>{T('新增账户')}
          </Button> */}
        </IceContainer>
        <Dialog language={T('zh-cn')} style={{width: '500px'}}
          ...
          footerAlign="center"
          footer={<view>
                    <Button style={styles.dialogOKBtn} type='primary' onClick={this.onSelfCreateAccountOK.bind(this)}>{T('确定')}</Button>
                    <Button style={styles.dialogCancelBtn} type='normal' onClick={this.onSelfCreateAccountClose.bind(this)}>{T('取消')}</Button>
                  </view>}
        >
        ...
        <Dialog language={T('zh-cn')}
          ...
          footerAlign="center"
          footer={<view>
                    <Button style={styles.dialogOKBtn} type='primary' onClick={this.onProxyCreateAccountOK.bind(this)}>{T('确定')}</Button>
                    <Button style={styles.dialogCancelBtn} type='normal' onClick={this.onProxyCreateAccountClose.bind(this)}>{T('取消')}</Button>
                  </view>}
        >
          <Input hasClear
            onChange={this.handleNewAccountNameChange.bind(this)}
        ...
        <Dialog language={T('zh-cn')}
          ...
          footerAlign="center"
          footer={<view>
                    <Button style={styles.dialogOKBtn} type='primary' onClick={this.onBindNewAuthorOK.bind(this)}>{T('确定')}</Button>
                    <Button style={styles.dialogCancelBtn} type='normal' onClick={this.onBindNewAuthorClose.bind(this)}>{T('取消')}</Button>
                  </view>}
        >
          <Input hasClear
        ...
        <Dialog language={T('zh-cn')}
          ...
          footerAlign="center"
          footer={<view>
                    <Button style={styles.dialogOKBtn} type='primary' onClick={this.onModifyThresholdOK.bind(this)}>{T('确定')}</Button>
                    <Button style={styles.dialogCancelBtn} type='normal' onClick={this.onModifyThresholdClose.bind(this)}>{T('取消')}</Button>
                  </view>}
        >
          <Select language={T('zh-cn')}
            style={{...styles.selectBoder, width: '100%'}}
        ...
        <Dialog language={T('zh-cn')}
          ...
          footerAlign="center"
          footer={<view>
                    <Button style={styles.dialogOKBtn} type='primary' onClick={this.onUpdateWeightOK.bind(this)}>{T('确定')}</Button>
                    <Button style={styles.dialogCancelBtn} type='normal' onClick={this.onUpdateWeightClose.bind(this)}>{T('取消')}</Button>
                  </view>}
        >
          <div>
            权重:
        ...
        >
          ...
          <Input hasClear 
            ...
            style={styles.inputBoder}
            innerBefore={this.state.feeTypeName}
            size="medium"
            defaultValue=""
            maxLength={16}
            hasLimitHint
        ...
        <Dialog language={T('zh-cn')}
          ...
          footer={<view>
                    <Button style={styles.dialogOKBtn} type='primary' onClick={this.onImportAccountOK.bind(this)}>{T('确定')}</Button>
                    <Button style={styles.dialogCancelBtn} type='normal' onClick={this.onImportAccountClose.bind(this)}>{T('取消')}</Button>
                  </view>}
        >
          <Input hasClear autoFocus
            onChange={this.handleImportAccountChange.bind(this)}
            style={styles.inputBoder}
            innerBefore={T("账号名/ID")}
            size="medium"
            defaultValue=""
            maxLength={50}
            hasLimitHint
        ...
        >
          <div className="editable-table">
            <IceContainer>
              <Table language={T('zh-cn')} primaryKey="owner" dataSource={this.state.authorList} hasBorder={false} isZebra={false} resizable>
        ...
        <Dialog language={T('zh-cn')}
          ...
          footer={<view>
                    <Button style={styles.dialogOKBtn} type='primary' onClick={this.onTransferOK.bind(this)}>{T('确定')}</Button>
                    <Button style={styles.dialogCancelBtn} type='normal' onClick={this.onTransferClose.bind(this)}>{T('取消')}</Button>
                  </view>}
        >
          <Input hasClear
          ...
          <Input hasClear
            ...
            innerAfter={this.state.transferAssetSymbol}
            size="medium"
            hasLimitHint
          />
        </Dialog>
        <Dialog language={T('zh-cn')} style={{ width: '500px'}}
          ...
          footer={<view>
                    <Button style={styles.dialogOKBtn} type='primary' onClick={this.onAddContractABIOK.bind(this)}>{T('确定')}</Button>
                    <Button style={styles.dialogCancelBtn} type='normal' onClick={this.onAddContractABIClose.bind(this)}>{T('取消')}</Button>
                  </view>}
        >
          <Input.TextArea hasClear autoHeight={{ minRows: 4, maxRows: 18 }} 
        ...
        <Dialog language={T('zh-cn')} style={{ width: '500px'}}
          ...
          footer={<view>
                    <Button style={styles.dialogOKBtn} type='primary' onClick={this.onAddContractByteCodeOK.bind(this)}>{T('确定')}</Button>
                    <Button style={styles.dialogCancelBtn} type='normal' onClick={this.onAddContractByteCodeClose.bind(this)}>{T('取消')}</Button>
                  </view>}
        >
